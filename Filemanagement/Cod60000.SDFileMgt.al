codeunit 60000 SDFileMgt
{
    ...
    begin
        if PersistentBlob.Exists(PID) then begin
            TempBlob.CreateOutStream(OutStr);//read the files
            PersistentBlob.CopyToOutStream(PID, OutStr);//takes the files into outstream
            TempBlob.CreateInStream(Instr);
            DownloadFromStream(Instr, '', '', '', Filename)
        end;
    end;
